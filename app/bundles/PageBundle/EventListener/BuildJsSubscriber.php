...
MauticJS.initGatedVideo = function () {
    ...
    if ("undefined" == typeof jQuery || "undefined" == typeof Froogaloop) {
        MauticJS.gatedVideoTimeoutID = window.setTimeout(MauticJS.initGatedVideo, 1000);
    } else {
        MauticJS.insertStyle('{$mediaElementCss}');

        {$mediaElementJs}
        
        MauticJS.processGatedVideos(MauticJS.videoElements);
    }
};

MauticJS.processGatedVideos = function (videoElements) {
    window.mediaPlayers  = [];
    ...
    MauticJS.addVideoView = function (i, totalWatched) {
        if (totalWatched == 0) {
            return;
        }
    ...
    MauticJS.endCurrentView = function (i) {
        var viewId = 0;

        // Find the first id of the view that is not locked and lock it
        MauticJS.iterateCollection(mediaPlayers[i].views)(function(node, j){
            if (mediaPlayers[i].views[j].locked == false) {
                viewId = j;
            }
